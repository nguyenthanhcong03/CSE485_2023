...
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th10 29, 2023 lúc 10:23 AM
...
CREATE TABLE `baiviet` (
  ...
  `tieude` varchar(200) NOT NULL DEFAULT '',
  `ten_bhat` varchar(100) NOT NULL DEFAULT '',
...
ALTER TABLE `baiviet`
  ...
  ADD KEY `FK_baiviet_tacgia` (`ma_tgia`),
  ADD KEY `FK_baiviet_theloai` (`ma_tloai`);
...
--
ALTER TABLE `baiviet`
  ADD CONSTRAINT `FK_baiviet_tacgia` FOREIGN KEY (`ma_tgia`) REFERENCES `tacgia` (`ma_tgia`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `FK_baiviet_theloai` FOREIGN KEY (`ma_tloai`) REFERENCES `theloai` (`ma_tloai`) ON DELETE NO ACTION ON UPDATE NO ACTION;
COMMIT;
